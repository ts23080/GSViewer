...
void main() {
    ...
    // --- 【超重要】アンチエイリアス補正の調整 ---
    // 0.3は少し強すぎて白っぽく（透けやすく）なるので、
    // 0.15程度に下げ、クッキリ感を維持しつつジャギを抑える。
    cov2D[0][0] += 1.0;
    cov2D[1][1] += 1.0;
    ...
    vOpacity = gs_in[0].opacity * 0.9;

    float mid = 0.5 * (cov2D[0][0] + cov2D[1][1]);
    float term = sqrt(max(0.01, mid * mid - det));
    float radius = ceil(3.0 * sqrt(max(0.01, mid + term))); 
