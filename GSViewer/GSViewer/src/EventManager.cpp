    ...
    m_camPos(0, 0, 0), m_camYaw(0), m_camPitch(0), m_camDist(10.0f)
{
    ...
    if (m_loader.LoadFromPly("model.ply")) {
        // 2. シェーダーのコンパイル
        GLuint program = CreateProgram("shaders/splat.vert", "shaders/splat.geo", "shaders/splat.frag");

        if (program != 0) {
            // 3. Rendererにデータを転送し、VAO/VBOを作成する
            m_renderer.Setup(m_loader.GetSplats(), program);
            std::cout << "EventManager: Setup complete. Shaders linked and data sent to GPU." << std::endl;
        }
        else {
            std::cerr << "EventManager: ERROR - Shader setup failed." << std::endl;
        }
    }
    else {
        std::cerr << "EventManager: ERROR - Could not load model.ply." << std::endl;
    }
}

void EventManager::DrawScene(int w, int h) {
    // 背景を少し明るいグレーにして、描画されているか分かりやすくする
    glViewport(0, 0, w, h);
